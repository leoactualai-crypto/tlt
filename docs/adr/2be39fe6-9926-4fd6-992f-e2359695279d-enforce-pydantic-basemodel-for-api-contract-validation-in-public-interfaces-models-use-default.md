...
1. SHOULD: Models SHOULD use default_factory for mutable defaults (dict, list) to prevent shared state bugs and use Field(default_factory=lambda: datetime.now(timezone.utc)) for timestamps.

## Policy Block

- SHOULD Models SHOULD use default_factory for mutable defaults (dict, list) to prevent shared state bugs and use Field(default_factory=lambda: datetime.now(timezone.utc)) for timestamps.
