...
1. MUST: Numeric fields from external sources MUST define range constraints using Field(ge=min, le=max) to enforce valid bounds

## Policy Block

- MUST Numeric fields from external sources MUST define range constraints using Field(ge=min, le=max) to enforce valid bounds
