...
1. SHOULD: Logging statements that include response data SHOULD use .get() to prevent log formatting failures when expected keys are absent

## Policy Block

- SHOULD Logging statements that include response data SHOULD use .get() to prevent log formatting failures when expected keys are absent
