...
1. MUST: FastAPI route handlers that interact with Discord bot instances MUST be declared as async functions to prevent blocking the web server

## Policy Block

- MUST FastAPI route handlers that interact with Discord bot instances MUST be declared as async functions to prevent blocking the web server
