...
1. MAY: Services MAY use synchronous route handlers only when the handler performs pure computation with no I/O and blocking behavior is explicitly acceptable

## Policy Block

- MAY Services MAY use synchronous route handlers only when the handler performs pure computation with no I/O and blocking behavior is explicitly acceptable
