...
1. MUST: Code accessing response dictionaries from httpx.AsyncClient, requests, or internal service calls MUST use .get() for all fields that are not guaranteed by API contract

## Policy Block

- MUST Code accessing response dictionaries from httpx.AsyncClient, requests, or internal service calls MUST use .get() for all fields that are not guaranteed by API contract
