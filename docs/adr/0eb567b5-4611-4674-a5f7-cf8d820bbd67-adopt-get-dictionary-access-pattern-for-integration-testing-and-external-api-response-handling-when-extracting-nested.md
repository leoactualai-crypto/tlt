...
1. SHOULD: When extracting nested dictionary values, code SHOULD chain .get() calls (e.g., state.get('lifecycles', {}).get(task_id)) to prevent AttributeError on None

## Policy Block

- SHOULD When extracting nested dictionary values, code SHOULD chain .get() calls (e.g., state.get('lifecycles', {}).get(task_id)) to prevent AttributeError on None
