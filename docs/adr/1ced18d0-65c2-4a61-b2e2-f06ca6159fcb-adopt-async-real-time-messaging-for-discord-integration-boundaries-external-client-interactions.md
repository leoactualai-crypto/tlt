...
1. MUST: External client interactions with the TLT service backend MUST use async HTTP clients (aiohttp.ClientSession, httpx.AsyncClient) to prevent blocking the Discord event loop

## Policy Block

- MUST External client interactions with the TLT service backend MUST use async HTTP clients (aiohttp.ClientSession, httpx.AsyncClient) to prevent blocking the Discord event loop
