...
1. MUST: Background tasks that poll external service state and dispatch messages MUST use async HTTP client sessions with explicit lifecycle management

## Policy Block

- MUST Background tasks that poll external service state and dispatch messages MUST use async HTTP client sessions with explicit lifecycle management
