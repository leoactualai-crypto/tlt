...
1. MUST: All real-time messaging operations at Discord integration boundaries MUST use async/await patterns with explicit await calls on send operations (channel.send(), thread.send(), user.send())

## Policy Block

- MUST All real-time messaging operations at Discord integration boundaries MUST use async/await patterns with explicit await calls on send operations (channel.send(), thread.send(), user.send())
