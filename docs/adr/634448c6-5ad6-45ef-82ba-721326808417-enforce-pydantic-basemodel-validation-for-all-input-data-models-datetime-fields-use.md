...
1. MUST: DateTime fields MUST use timezone-aware datetime objects with default_factory=lambda: datetime.now(timezone.utc) for timestamp generation

## Policy Block

- MUST DateTime fields MUST use timezone-aware datetime objects with default_factory=lambda: datetime.now(timezone.utc) for timestamp generation
