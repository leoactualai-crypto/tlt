...
1. MUST: All HTTP requests to external clients MUST specify an explicit timeout parameter (e.g., requests.get(url, timeout=30)) to prevent resource exhaustion

## Policy Block

- MUST All HTTP requests to external clients MUST specify an explicit timeout parameter (e.g., requests.get(url, timeout=30)) to prevent resource exhaustion
