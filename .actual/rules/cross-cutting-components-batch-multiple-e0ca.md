...
- **R-ASYNC-001** MAY: Components MAY batch multiple asynchronous send operations using task groups or gather patterns when order independence permits.
...
- **R-ASYNC-003** SHOULD: For operations that coordinate multiple sends, prefer structured concurrency patterns that group related tasks and propagate cancellation consistently.
- **R-ASYNC-004** MUST: Add timeout parameters to all external send operations to prevent indefinite hangs; choose timeout values based on platform SLA documentation and user experience requirements.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All async send operations MUST be verified against the integration test suite and static analysis linter before merge. Blocking calls in async contexts are automatically flagged violations requiring remediation.
</enforcement>