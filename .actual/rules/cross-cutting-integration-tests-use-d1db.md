# Adopt .get() Dictionary Access Pattern for Integration Testing and External API Response Handling: Integration Tests Use
...
- **R-DICT-001** SHOULD: Integration tests SHOULD use .get() with explicit default values to document expected response structure and handle missing fields gracefully.
...
- **R-DICT-004** SHOULD: When logging response data, SHOULD use .get() to prevent log formatting failures: logger.info(f"Guild: {result.get('guild_id', 'unknown')}").
- **R-DICT-005** SHOULD: For CloudEvent data extraction, SHOULD use .get() since event data structure varies by event type: event.get('guild_id'), event.get('message_id').
...
<enforcement>
Verification is mandatory. Code review MUST enforce .get() usage for all external API response handling. CI pipeline MUST fail if grep detects direct dictionary key access patterns in new code touching external API boundaries. Production KeyError exceptions MUST trigger incident review to add .get() usage and integration test coverage. Exceptions MUST be documented in code comments with explicit justification and logged in ADR exceptions registry.
</enforcement>