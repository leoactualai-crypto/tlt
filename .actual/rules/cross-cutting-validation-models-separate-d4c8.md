# Adopt Pydantic BaseModel for Domain Validation in API Boundaries: Validation Models Separate
...
- **R-PYDANTIC-001** SHOULD: Validation models SHOULD separate concerns between create/input models (e.g., ReminderCreate, ExperienceCreate) and response/output models (e.g., ReminderResponse, ExperienceResponse).
...
<enforcement>
Clause Code MUST NOT skip or defer verification. All new API endpoints MUST declare Pydantic validation models with explicit input/output separation. CI pipeline MUST fail if new router endpoints lack response_model declarations or use untyped dictionaries for validation.
</enforcement>