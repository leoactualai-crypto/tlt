# Enforce Pydantic BaseModel Validation for Public API Contracts: Public Request Models

These rules are ALWAYS ACTIVE for all public API request models, FastAPI router endpoints, MCP service interfaces, and inter-service data contracts within the monorepo/tlt system.

### Rules

- **R-PYDANTIC-001** MUST: All public API request models MUST inherit from `pydantic.BaseModel` and define explicit field types with type annotations.
...
- All public API request models in scope files inherit from BaseModel with explicit type annotations

<enforcement>
Claude Code MUST NOT skip or defer verification. All new API endpoints and service contracts MUST comply with R-PYDANTIC-001 through R-PYDANTIC-005. Code review MUST block merge until BaseModel validation is added or a documented exception (EXC-001 or EXC-002) is approved. CI pipeline MUST fail if violations are detected.
</enforcement>