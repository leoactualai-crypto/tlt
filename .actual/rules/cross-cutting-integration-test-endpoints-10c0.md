...
These rules are ALWAYS ACTIVE for all integration testing involving real-time messaging boundaries, specifically HTTP endpoints that trigger real-time message delivery, integration test routers exposing reminder scheduling and thread management, and asynchronous operations coordinating HTTP responses with message sending.

### Rules

- **R-ASYNC-001** MUST: Integration test endpoints that expose real-time messaging operations MUST use asynchronous function definitions to coordinate HTTP request handling with asynchronous message delivery.
...
- Static analysis confirms no synchronous function definitions in integration test modules that invoke real-time messaging clients

<enforcement>
Clause Code MUST NOT skip or defer verification. Integration tests that fail due to synchronous coordination or missing await syntax block pull request merges until corrected. Code review must verify asynchronous function definitions. Static analysis tools must detect violations. Violations require refactoring to asynchronous coordination patterns or documented exception approval from engineering team lead.
</enforcement>