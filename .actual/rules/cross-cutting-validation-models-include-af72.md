# Standardize Pydantic BaseModel for Input Validation in Logged Service Endpoints: Validation Models Include
...
- **R-VAL-001** SHOULD: Validation models SHOULD include description metadata via `Field(description=...)` to document field semantics for both validation and logging context.
...
<enforcement>
Clause Code MUST NOT skip or defer verification. All new FastAPI endpoints with logging require Pydantic BaseModel validation models with Field descriptions. CI pipeline MUST fail if router endpoints lack BaseModel definitions or response_model parameters. Code review MUST block merges missing Field constraints for numeric fields.
</enforcement>