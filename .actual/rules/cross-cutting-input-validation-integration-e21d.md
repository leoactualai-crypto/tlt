# Adopt Async Real-Time Messaging for Discord Integration Boundaries: Input Validation Integration
...
- **R-DISCORD-001** MUST: Input validation at integration boundaries MUST use Pydantic BaseModel schemas (ReminderCreate, ReminderResponse) to enforce type safety and validation rules before async operations.
...
# Verify async function definitions for reminder and lifecycle operations
...
# Verify Pydantic BaseModel schemas for integration boundaries
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All four verification commands must return non-zero counts indicating async patterns and Pydantic validation are present in the Discord adapter layer.
</enforcement>